...
enum Rank: Int {
    ...
    static let allRanks: [Rank] = [Rank.ace,Rank.two,Rank.three,Rank.four,Rank.five,Rank.six,Rank.seven,Rank.eight,Rank.nine,Rank.ten,Rank.jack,Rank.queen,Rank.king]
}
...
extension Rank: CustomStringConvertible {
    var description: String {
        return "(\(Rank.ace.rawValue),\(Rank.two.rawValue),\(Rank.three.rawValue),\(Rank.four.rawValue),\(Rank.five.rawValue),\(Rank.six.rawValue),\(Rank.seven.rawValue),\(Rank.eight.rawValue),\(Rank.nine.rawValue),\(Rank.ten.rawValue),\(Rank.jack.rawValue),\(Rank.queen.rawValue),\(Rank.king.rawValue))"
    }
}
...
enum Suit: String {
    ...
    static let allSuits: [Suit] = [Suit.heart,Suit.diamond,Suit.spade,Suit.cube]
}
...
class HighLow: CardGame {
    ...
    func play() {
        let cardDraw1 = deck.drawCard()
        let cardDraw2 = deck.drawCard()
        //print(cardDraw2)
        //print(cardDraw1)
        if cardDraw1 < cardDraw2 {
            print("Player 2 wins with \(cardDraw2.rank.rawValue) of \(cardDraw2.suit.rawValue)")
        } else if cardDraw2 < cardDraw1 {
            print("Player 1 wins with \(cardDraw1.rank.rawValue) of \(cardDraw1.suit.rawValue)")
        } else {
            print("Round ends in a tie with \(cardDraw2.rank.rawValue) of \(cardDraw2.suit.rawValue)")
        }
        //delegate?.gameDidStart(_: CardGame)
        delegate?.game(player1DidDraw: cardDraw1, player2DidDraw: cardDraw2)
    }
...
class CardGameTracker: CardGameDelegate {
    func game(player1DidDraw card1: Card, player2DidDraw card2: Card) {
        print("Player 1 drew a \(card1.rank.rawValue) of \(card1.suit.rawValue), player 2 drew a \(card2.rank.rawValue) of \(card2.suit.rawValue)")
    }
...
//: ```
let x = HighLow(delegate: CardGameTracker() as CardGameDelegate)
x.play()
