...
extension Rank: CustomStringConvertible {
    var description: String {
        return "(You have \(Rank.ace) with is \(Rank.ace.rawValue)"
    }
}
...
struct Deck {
    ...
    init(cards: [Card]) {
        ...
        // step 9
        for cardRank in cards {
            for cardSuite in cards {
                print("I have a \(cardRank.rank) of \(cardSuite.suit)")
            }
        }
    }
...
//    let newDeck = Deck()
    ...
    // Step 11
    func drawCard() -> Int {
        let randomCard = Int.random(in: 0...50)
        return randomCard
    }
}
...
// Coming Back to This step Look to Step 6
let myDeck = Deck()
...
class HighLow: CardGame {
    ...
    init(deck: Int, delegate: CardGameDelegate?) {
        self.deck = deck
...
extension Rank: Comparable {
    static func < (lhs: Rank, rhs: Rank) -> Bool {
        ...
        } else if lhs.allRanks != rhs.allRanks {
            return lhs.allRanks < rhs.allRanks
        }
    }
