...
struct Card{
    let cardSuit: PlayingCardSuit
    let cardRank: PlayingCardRank
}
//: ## Step 5
//: Make the card also conform to `CustomStringConvertible`. When turned into a string, a card's value should look something like this, "ace of spades", or "3 of diamonds".
extension Card: CustomStringConvertible{
    var description: String{
        return "\(cardRank) of \(cardSuit.rawValue)"
    }
}
//: ## Step 6
...
extension PlayingCardRank{
    static let allRanks: () -> [PlayingCardRank] = {
...
extension PlayingCardSuit{
    static let allSuits: () -> [PlayingCardSuit] = {
...
extension Deck{
    init(){
        var cards: [Card] = []
        let playingCardSuits: [PlayingCardSuit] = PlayingCardSuit.allSuits()
        let playingCardRanks: [PlayingCardRank] = PlayingCardRank.allRanks()
//: ## Step 10
//: These loops will allow you to match up every rank with every suit. Make a `Card` object from all these pairings and append each card to the `cards` property of the deck. At the end of the `init` method, the `cards` array should contain a full deck of standard playing card objects.
    for cardRank in playingCardRanks{
        for cardSuit in playingCardSuits{
            cards.append(Card(cardSuit: cardSuit, cardRank: cardRank))
        }
    }
    self.cards = cards
}
 ...
 extension PlayingCardRank{
     static func < (lhs: PlayingCardRank, rhs: PlayingCardRank) -> Bool{
         if (lhs.rawValue < rhs.rawValue){
             return lhs.self == .Ace ? false : true
         }
         return false
     }
     static func == (lhs: PlayingCardRank, rhs: PlayingCardRank) -> Bool{
...
extension Card: Comparable{
    ...
    static func == (lhs: Card, rhs: Card) -> Bool {
        if lhs.cardRank.self == rhs.cardRank.self{
            if lhs.cardSuit.rawValue == rhs.cardSuit.rawValue{
                return true
            }
        }
        return false
...
class HighLow: CardGame{
    ...
    func play(){
        ...
        let player1: Card = deck.drawCard()
        let player2: Card = deck.drawCard()
        cardGameDelegate?.game(player1DidDraw: player1, player2DidDraw: player2)
        if (player1.cardRank == player2.cardRank){
            print("Ends in a tie with a \(player1)")
        }else{
...
//: ```
let game = HighLow()
game.cardGameDelegate = CardGameTracker()
game.play()
