...
enum CardSuit: String {
    ...
    static var allSuits: [CardSuit] = [.clubs, .diamonds, .spades, .hearts ]
...
struct Deck {
    ...
    func draw() -> Card {
        return cards [Int.random(in: 1...cards.count)]
    }
}
...
class HighLow: CardGame {
    ...
    func play() {
        if let player1 = deck.drawCard(),
            ...
            if player1 > player2 {
                print("Player 1 wins, with card \(player1.description)")
            } else if player2 > player1 {
                print("Player 2 wins, with card \(player2.description)")
            } else if player2 == player1 {
...
class CardGameTracker: CardGameDelegate {
    ...
    func gameDidStart(_ game: CardGame) {
        if game is HighLow {
            print("Game of HighLow starts.")
        }
    }
    
    func game(player1DidDraw card1: Card, player2DidDraw card2: Card) {
        print("Player 1 has drawn \(card1). Player 2 has drawn \(card2).")
    }
}
