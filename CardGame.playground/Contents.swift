...
protocol CardGameDelegate {
    ...
    func game(player1DidDraw firstCard: Card, player2DidDraw secondCard: Card)
...
//: As part of the protocol conformance, implement a method called `play()`. The method should draw 2 cards from the deck, one for player 1 and one for player 2. These cards will then be compared to see which one is higher. The winning player will be printed along with a description of the winning card. Work will need to be done to the `Suit` and `Rank` types above, so see the next couple steps before continuing with this step.
    ...
    func play() {
        ...
        delegate?.game(player1DidDraw: playerOne, player2DidDraw: playerTwo)
        
        if playerOne.rank.rawValue > playerTwo.rank.rawValue {
            print("Player One wins with \(playerOne.suit.rawValue) \(playerOne.rank.rawValue)")
        } else if playerOne.rank.rawValue < playerTwo.rank.rawValue {
            print("Player Two wins with \(playerTwo.suit.rawValue) \(playerTwo.rank.rawValue)")
        } else {
            print("It's a tie.")
        }
    }
...
class CardGameTracker: CardGameDelegate {
    ...
    func game(player1DidDraw firstCard: Card, player2DidDraw secondCard: Card) {
        print("Player 1 drew a(n) \(firstCard)\nPlayer 2 drew a(n) \(secondCard)")
    }
