...
struct Deck {
    ...
    init(){
        ...
        for suit in Suit.allSuits {
            for rank in Rank.allRanks {
                tempDeck.append(Card(rank: rank, suit: suit))
            }
        }
    ...
    func drawCard() -> Card {
        let randomNumber: Int = Int.random(in: 0...51)
        return deckOfCards[randomNumber]
    }
...
//
    ...
    var delegate: CardGameDelegate?
     ...
     init(deck: Deck, cardGameDelegate: CardGameDelegate?) {
           self.deck = deck
           self.delegate = cardGameDelegate
       }
}
...
class CardGameTracker: CardGameDelegate {
    ...
    func gameDidStart(cardGame: CardGame) {
        ...
        if cardGame is HighLow {
                print("Started a new High Low game.")
            }
        }
    
    func game(player1DidDraw card1: Card, player2DidDraw card2: Card) {
        print("Player1 drew a \(card1.description) and Player2 drew a \(card2.description)")
    }
...
//: ```
var cardGameTracker = CardGameTracker()
var highLow = HighLow(deck: Deck(), cardGameDelegate: cardGameTracker)
highLow.play()
