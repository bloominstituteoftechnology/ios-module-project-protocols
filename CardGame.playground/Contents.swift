    ...
    init() {
        for rank in PlayingCard.allCards {
            for suit in CardSuit.allSuits {
                switch suit {
                case .hearts: cardArray.append(Card(suit: suit, rank: rank))
                case .spades: cardArray.append(Card(suit: suit, rank: rank))
                case .clubs: cardArray.append(Card(suit: suit, rank: rank))
                case .diamonds: cardArray.append(Card(suit: suit, rank: rank))
            }
        }
    }
}
...
class HighLow: CardGame {
    ...
    func play() {
        ...
        if card1 == card2 {
            print("Round ends in a tie with \(card1.rank) of \(card1.suit)")
        } else if card2 < card1 {
            print("Player 1 wins the round with \(card1.rank) of \(card1.suit)")
        } else if card1 < card2 {
            print("Player 2 wins the round with \(card2.rank) of \(card2.suit)")
            }
        }
...
let deck1 = DeckOfCards()
let newTracker = CardGameTracker()
let newGame = HighLow(getDeck: deck1, delegate: newTracker)
