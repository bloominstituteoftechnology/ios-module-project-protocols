...
struct Card: CustomStringConvertible, Comparable{
    ...
    static func == (lhs: Card, rhs: Card) -> Bool {
        return lhs.rank == rhs.rank && lhs.suits == rhs.suits
    }
    ...
    var description: String{
        get{
            return "\(rank.rawValue) of \(suits.rawValue)"
        }
    }
}
...
struct Deck{
    ...
    init() {
        ...
        for rank in Rank.allRanks{
            for suit in Suits.allSuits{
                let cards = Card(rank: rank, suits: suit)
                card.append(cards)
            }
        }
        self.card = card
    ...
    func drawCard() -> Card {
        let randomCard = Int.random(in: 1...51)
...
class HighLow: CardGame{
    ...
    func play() {
        ...
        if player1 == player2{
            print("Rounds ends in a tie with Player 1: \(player1.rank.rawValue) of \(player1.suits.rawValue)")
        } else if player1 > player2{
            print("Player 1 wins with \(player1.rank.rawValue) of \(player1.suits.rawValue)")
        }else{
            print("Player 2 wins with \(player2.rank.rawValue) of \(player2.suits.rawValue) ")
        }
    }
