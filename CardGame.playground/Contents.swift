...
enum PlayingCard: Int, Comparable {
...
enum PlayingCardSuit: String {
    ...
    static var allSuits: [PlayingCardSuit] {
        return [.hearts, .diamond, .spades, .clubs]
    }
}
...
struct Deck {
    ...
    init() {
        ...
        for card in PlayingCard.allRanks {
            for suit in PlayingCardSuit.allSuits{
                let thisCard: Card = Card(suit: PlayingCardSuit(rawValue: suit.rawValue)!, rank: PlayingCard(rawValue: card.rawValue)!)
                deckOfCards.append(thisCard)
            }
        }
    ...
    func drawCard() -> Card {
        let randomCard = Int.random(in: 0...deck.count-1)
        return deck[randomCard]
    }
}

let thisDeck: Deck = Deck()
    ...
    class HighLow: CardGame {
        ...
        func play() {
            ...
            if player1Card.rank == player2Card.rank {
                print("Round ends in a draw with \(player1Card.rank)!")
            } else if player1Card.rank > player2Card.rank {
                print("Player 1 wins with \(player1Card.rank)!")
            } else if player1Card.rank < player2Card.rank {
                print("Player 2 wins with \(player2Card.rank)!")
            } else {
