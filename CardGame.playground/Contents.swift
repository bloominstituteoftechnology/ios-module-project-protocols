...
// - Checking to see if it is working so far
let card = (Card(suit: .clubs, rank: .jack))
...
// append on number 6 towards bottom
...
//: As part of the protocol conformance, implement a method called `play()`. The method should draw 2 cards from the deck, one for player 1 and one for player 2. These cards will then be compared to see which one is higher. The winning player will be printed along with a description of the winning card. Work will need to be done to the `Suit` and `Rank` types above, so see the next couple steps before continuing with this step.
    func play() {
            if let player1 = deck.drawCard(),
            let player2 = deck.drawCard() {
            if player1 > player2 {
                print("Player 1 wins, \(player1.description) beats \(player2.description)")
            } else if player2 > player1{
                print("Player 2 wins, \(player2.description) beats \(player1.description)")
            } else if player1 == player2 {
                print("It's a draw, both players have \(player1.description)")
            } else {
                print("It's a draw, \(player1.description) and \(player2.description)")
            }
        } else {
...
class CardGameTracker: CardGameDelegate {
    ...
    func gameDidStart(game: CardGame) {
        if game is HighLow {
            print("Started a new gane of High Low")
        }
    }
    
    func game(player1DidDraw card1: Card, player2DidDraw card2: Card) {
        print("Playe 1 drew a \(card1), player 2 drew a \(card2).")
    }
}
//: Step 21
