...
class HighLow: CardGame {
    ...
    func play() {
        ...
        let p1c = deck.drawCard()
        let p2c = deck.drawCard()
        delegate?.game(player1DidDraw: p1c, player2DidDraw: p2c)
        if p1c.rank == p2c.rank { print("Round ends in a tie of \(p1c.rank)s.") }
        else if p1c > p2c { print("Player 1 wins with \(p1c).") }
        else if p1c < p2c { print("Player 2 wins with \(p2c).") }
    }
}
//: ## Step 20
...
class CardGameTracker: CardGameDelegate {
    func gameDidStart(_ cardGame: CardGame) {
        print("Started a new game of \(cardGame.name).")
    }
    func game(player1DidDraw card1: Card, player2DidDraw card2: Card) {
