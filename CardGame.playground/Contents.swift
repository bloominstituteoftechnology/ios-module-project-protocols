...
extension Deck {
    init(){
        ...
        for aRank in Rank.allRanks{
            for aSuit in Suit.allSuits{
                let newCard = Card(rank: Rank(rawValue: aRank.rawValue)!, suit: Suit(rawValue: aSuit.rawValue)!)
...
extension Deck {
    func drawCard() -> Card{
        let randomCard = Int.random(in: 0...deckOfCards.count-1)
