    ...
    init() {
        ...
        for suit in Suit.allSuits() {
            for rank in Rank.allRanks() {
                self.theDeck.append(Card(suit: suit, rank: rank))
            }
        }
    }
    
    func drawCard() -> Card {
        return theDeck[Int.random(in: 0...51)]
    }
}
...
class HighLow: CardGame {
    ...
    //draw 2 cards from the deck, one for player 1 and one for player 2.
    //These cards will then be compared to see which one is higher.
    //The winning player will be printed along with a description of the winning card.
    func play() {
        ...
        self.delegate?.game(player1DidDraw: player1Card, player2DidDraw: player2Card)
...
//: ```
let theGame = HighLow()
theGame.delegate = CardGameTracker()
