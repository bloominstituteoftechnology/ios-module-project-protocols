...
enum Rank: Int, CustomStringConvertible, Comparable {
    ...
    static var  ranks: [Rank] { return [.ace, .two, .three, .four, .five, .six, .seven, .eight, .nine, .ten, .jack, .queen, .king]
...
struct Deck {
    ...
    init(){
        for s in SuitEnum.suits {
            for r in Rank.ranks {
                let singleCard = Card(suit: s, rank: r)
                
                card.append(singleCard)
    ...
    func drawCard()-> Card {
        let randomNumber = Int.random(in: 1...card.count) - 1
...
class HighLow: CardGame {
    var deckVariable: Deck = Deck()
    var del: CardGameDelegate?
    
    func play() {
        del?.gameDidStart(self)
        let player1Card: Card = deckVariable.drawCard()
        let player2Card: Card = deckVariable.drawCard()
        del?.game(player1DidDraw: player1Card, player2DidDraw: player2Card)
        if player1Card < player2Card {
            print(" Player 2 wins with the winning card of \(player2Card.description)")
            
        } else if player1Card == player2Card {
            print("Round ends in a tie with \(player1Card.description)")
        } else {
            print(" Player 1 wins with the winning card of \(player1Card.description)")
        }
...
class CardGameTracker: CardGameDelegate {
    ...
    func game(player1DidDraw card1: Card, player2DidDraw card2: Card) {
        print("Player 1 drew \(card1.description), player 2 drew \(card2.description)")
    }
...
//: ```
let playGame = HighLow()
playGame.del = CardGameTracker()
playGame.play()
